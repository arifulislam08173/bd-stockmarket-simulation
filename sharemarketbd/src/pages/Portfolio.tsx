...
import { TrendingUp, TrendingDown, Briefcase, ArrowUpRight, ArrowDownRight } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container py-6 space-y-6 animate-fade-in">
        <div>
          <h2 className="text-2xl font-display font-bold text-foreground">My Portfolio</h2>
          <p className="text-muted-foreground text-sm">Manage your stock holdings</p>
        </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border bg-muted/50">
                      ...
                      <th className="text-right py-3 px-4 font-semibold text-muted-foreground">Avg Price</th>
                      <th className="text-right py-3 px-4 font-semibold text-muted-foreground">Current</th>
                  ...
                  <tbody>
                    {portfolio.map((item) => {
                      ...
                      return (
                        <tr key={item.symbol} className="border-b border-border hover:bg-muted/30 transition-colors">
                          ...
                          <td className="text-right py-4 px-4 font-mono font-medium text-foreground">
                            ৳{item.totalValue.toLocaleString('en-BD', { minimumFractionDigits: 2 })}
                          </td>
                          <td className="text-right py-4 px-4">
                            <div className={cn(
                              "inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium",
                              isPositive ? "bg-stock-up-bg text-stock-up" : "bg-stock-down-bg text-stock-down"
                            )}>
                              {isPositive ? (
